    ...
    return (
        <div className='group relative w-full 2xl:w-[350px] dark:text-gray-300 border border-teal-500 hover:border-2 h-[400px] overflow-hidden rounded-lg sm:w-[430px] transition-all mx-auto'>
            ...
            <div className="p-3 flex flex-col gap-2">
                <p className='text-lg font-semibold line-clamp-2 group-hover:text-white'>{post.title}</p>
                <span className='italic text-sm'>{post.category === 'undefined' ? 'uncategorized' : post.category}</span>
                <Link
                    to={`/post/${post._id}`}
